...
import { verifyInputArtifacts } from "../src/inputArtifactChecks";
...
const inputArtifactChecks = verifyInputArtifacts();
...
const forbiddenFailed = candidateResults.some((result) => result.forbidden_authority_reachable);
const terminalFailed = candidateResults.some((result) => !result.terminal_expectations_satisfied);
const soundnessStatus = forbiddenFailed
  ? "fail_forbidden_authority_reachable"
  : terminalFailed
    ? "fail_terminal_expectation_mismatch"
    : "pass_bounded_public_reachability_scope";
const readout = {
  ...
  input_chain: [
    ...
    "phase2_dep_001_authorized_fork_push_draft_pr_observed"
  ],
  input_artifact_checks: inputArtifactChecks,
  ...
  ...invariantReadout,
  soundness_status: soundnessStatus,
  non_claims: {
    bpmn_engine: false,
    dmn_engine: false,
